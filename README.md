...
본 프로젝트는 소프트웨어 공학 수업에서 진행한 학술 프로젝트로,
UI 설계, 유스케이스/시퀀스/클래스 다이어그램, DFD, 아키텍처 설계, 비용 산정, 위험 관리 등
소프트웨어 전반의 설계 및 분석 업무를 수행하였습니다.
설계 단계에 중점을 두었으며 실제 프로그램 제작은 범위에 포함하지 않습니다.
...
| 팀원 A    | 유스케이스 다이어그램 및 시나리오 작성 |
| 팀원 B    | 클래스 다이어그램, 아키텍처 설계 |
| 팀원 C    | UI 기본 정책 수립, 플로우 차트 작성, DFD |
...
교수님께서 제공한 기능점수 산정 양식을 활용하여
전체 시스템 구현 시 예상되는 개발비용을 추정하였습니다.
...
유스케이스 시나리오는 별도의 PDF 문서로 정리되어 있으며,
아래 링크에서 확인할 수 있습니다.
